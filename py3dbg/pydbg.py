...
class pydbg:
    ...
    def debug_active_process (self, pid):
        ...
        if not ntdll.NtDebugActiveProcess(DWORD(pid)) :
        # if not kernel32.DebugActiveProcess  (DWORD(pid)):
    ...
    def disasm (self, address):
        ...
        # update our internal member variables.
        instructions = list(Cs(CS_ARCH_X86, CS_MODE_64).disasm(data, 32))
        ...
        if not self.instruction:
            ...
            self.op1 = self.instruction.op_str.split(", ")[0] if "," in self.instruction.op_str else self.instruction.op_str
            self.op2 = self.instruction.op_str.split(", ")[1] if ", " in self.instruction.op_str else ""
            self.op3 = self.instruction.op_str.split(", ")[2] if ", " in self.instruction.op_str and len(self.instruction.op_str.split(", ")) == 3 else ""

            # the rstrip() is for removing extraneous trailing whitespace that libdasm sometimes leaves.
            return "{} {}".format(self.instruction.mnemonic, self.instruction.op_str)#pydasm.get_instruction_string(self.instruction, pydasm.FORMAT_INTEL, address).rstrip(" ")
    ...
    ####################################################################################################################
    def dump_context (self, context=None, stack_depth = 0x10, print_dots=True):
        '''
        ...
        if type(context).__name__ != "_CONTEXT64" :
            ...
            # x64
            context_dump += "  RIP: %#016x %s\n" % (context.Rip, context_list["rip"])
            context_dump += "  RAX: %#016x (%20d) -> %s\n" % (context.Rax, context.Rax, context_list["rax"])
            context_dump += "  RBX: %#016x (%20d) -> %s\n" % (context.Rbx, context.Rbx, context_list["rbx"])
            context_dump += "  RCX: %#016x (%20d) -> %s\n" % (context.Rcx, context.Rcx, context_list["rcx"])
            context_dump += "  RDX: %#016x (%20d) -> %s\n" % (context.Rdx, context.Rdx, context_list["rdx"])
            context_dump += "  RDI: %#016x (%20d) -> %s\n" % (context.Rdi, context.Rdi, context_list["rdi"])
            context_dump += "  RSI: %#016x (%20d) -> %s\n" % (context.Rsi, context.Rsi, context_list["rsi"])
            context_dump += "  RBP: %#016x (%20d) -> %s\n" % (context.Rbp, context.Rbp, context_list["rbp"])
            context_dump += "  RSP: %#016x (%20d) -> %s\n" % (context.Rsp, context.Rsp, context_list["rsp"])

            for offset in range(0, stack_depth + 1):
                context_dump += "  +%#02x: %#16x (%10d) -> %s\n" %    \
                (                                                   \
                    offset * 8,                                     \
                    context_list["rsp+%#02x" % (offset * 8)]["value"],   \
                    context_list["rsp+%#02x" % (offset * 8)]["value"],   \
                    context_list["rsp+%#02x" % (offset * 8)]["desc"]     \
                )
    ...
    def dump_context_list (self, context=None, stack_depth=5, print_dots=True, hex_dump=False):
        ...
        if type(context).__name__ != "_CONTEXT64" :
            ...
            # x64
            context_list["rip"] = self.disasm(context.Rip) # self.smart_dereference(context.Rax, print_dots, hex_dump)
            ...
            context_list["rcx"] = self.smart_dereference(context.Rcx, print_dots, hex_dump)
            context_list["rdx"] = self.smart_dereference(context.Rdx, print_dots, hex_dump)
            context_list["rdi"] = self.smart_dereference(context.Rdi, print_dots, hex_dump)
            ...
            for offset in range(0, stack_depth + 1):
                try:
                    rsp = self.flip_endian_dword(self.read_process_memory(context.Rsp + offset * 8, 8))
                    context_list["rsp+%#02x" % (offset * 8)]          = {}
                    context_list["rsp+%#02x" % (offset * 8)]["value"] = rsp
                    context_list["rsp+%#02x" % (offset * 8)]["desc"]  = self.smart_dereference(rsp, print_dots, hex_dump)
                except:
                    context_list["rsp+%#02x" % (offset * 4)]          = {}
                    context_list["rsp+%#02x" % (offset * 4)]["value"] = 0
                    context_list["rsp+%#02x" % (offset * 4)]["desc"]  = "[INVALID]"
    ...
    def get_thread_context (self, thread_handle=None, thread_id=0):
        ...
        if not is_64bits or process_is_wow64(self.pid) :
    ...
    def get_thread_context32 (self, thread_handle=None, thread_id=0):
        ...
        if not kernel32.Wow64GetThreadContext(h_thread, byref(context)):
    ...
    def load (self, path_to_file, command_line=None, create_new_console=False, show_window=True):
        ...
        if not is_64bits or process_is_wow64(self.pid) :
            ...
            GetThreadSelectorEntry = kernel32.Wow64GetThreadSelectorEntry if is_64bits else kernel32.GetThreadSelectorEntry

            if not GetThreadSelectorEntry (pi.hThread, thread_context.SegFs, byref(selector_entry)):
                self.win32_error("GetThreadSelectorEntry ()")
            ...
            from .custom_module import get_process_teb
            teb, peb = get_process_teb(pi.hProcess)
            self.peb = peb
    ...
    def win32_error (self, prefix=None):
        ...
        kernel32.FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM,
                                ...
                                None)
        if prefix:
            error_message = "%s: %s" % (prefix, error.value.decode('euc-kr'))
        else:
            error_message = "GetLastError(): %s" % error.value.decode('euc-kr')
